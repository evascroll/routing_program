...
stations_under_construction = ["Landsdowne", "Olympic Village"]
...
def set_start_and_end(start_point, end_point):
    if start_point is not None:
        change_point = input(
            "What would you like to change? You can enter 'o' for 'original', 'd' for 'destination' or 'b' for 'both': "
        )
        if change_point == "b":
...
def show_landmarks():
    see_landmarks = input(
        "Woulld you like to see the list of landmarks again? Enter y/n: "
    )
    if see_landmarks == "y":
...
def new_route(start_point=None, end_point=None):
    ...
    if shortest_route is not None:
        ...
        print(
            "Unfortunately, there is no path between {0} and {1} due to maintenance.".format(
...
def get_route(start_point, end_point):
    ...
    for start_station in start_stations:
        for end_station in end_stations:
            metro_system = (
                get_active_station() if stations_under_construction else vc_metro
            )
            if len(stations_under_construction) > 0:
                posible_route = dfs(metro_system, start_station, end_station)
                if posible_route is None:
                    return None
